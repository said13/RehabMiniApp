...
export default function RehabMiniApp() {
  ...
  const selectedTraining = useMemo(
    () => selectedCategory?.trainings.find((t) => t.id === selectedTrainingId) || null,
    [selectedCategory, selectedTrainingId],
  );
  ...
  const startExercise = (exerciseId: string) => {
    ...
    const exercise = selectedTraining.complexes
      .flatMap((c) => c.exercises)
      .find((e) => e.id === exerciseId);
    if (!exercise) return;
    const single: Training = {
      id: `${selectedTraining.id}-${exercise.id}`,
      title: exercise.title,
      complexes: [{ id: exercise.id, title: exercise.title, exercises: [exercise] }],
    };
  ...
  return (
    ...
    >
      <main className="flex-1 pb-20 animate-fadeIn">
        {tab === 'home' && (
          <div>
            {!selectedCategoryId && !selectedTrainingId && (
              <CategoriesScreen
                categories={categories}
                banners={banners}
                activeBannerIndex={bannerIdx}
                onSelectBanner={setBannerIdx}
            ...
            {selectedTraining && (
              <ExercisesScreen
                ...
                onStartWorkout={(_) => setViewerCourse(selectedTraining)}
